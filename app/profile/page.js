          ...
          <div className="mb-10">
            <div className="bg-gradient-to-br from-blush-light via-cream to-honey-light/30 rounded-3xl border-2 border-blush/20 p-6 md:p-8 relative overflow-hidden">
              ...
              <div className="relative flex flex-col md:flex-row items-center gap-6">
                ...
                {/* Content */}
                <div className="flex-1 text-center md:text-left">
                  <p className="text-xs font-bold text-blush uppercase tracking-[0.2em] mb-1">
                    Official Boarding Pass
                  </p>
                  <h3 className="text-lg md:text-xl font-bold text-ink mb-2">
                    Ferry Ticket #
                    {user._id?.slice(-6).toUpperCase() || "000000"}
                  </h3>
                  <p className="text-sm text-ink-soft mb-4">
                    <span className="font-medium text-ink">{user.name}</span>{" "}
                    embarked on their journaling journey
                    <span className="font-medium text-ink">
                      {" "}
                      {getJoinDate()}
                    </span>
                    . This certifies their commitment to capturing life's
                    beautiful moments.
                  </p>
                  ...
                  <button
                    onClick={() => {
                      ...
                      ctx.fillStyle = "#8B7355";
                      ctx.fillText(
                        `TICKET #${user._id?.slice(-6).toUpperCase() || "000000"}`,
                        400,
                        420,
                      );
