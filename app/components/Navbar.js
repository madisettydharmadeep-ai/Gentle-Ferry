...
export default function Navbar() {
  ...
  return (
    <nav className="flex items-center justify-between px-3 md:px-10 h-[52px] bg-white/20 backdrop-blur-3xl border-b border-white/20 sticky top-0 z-[100] shadow-xl relative transition-all">
      ...
      <Link
        href="/dashboard"
        className={`flex items-center gap-2 font-bold text-lg no-underline hover:opacity-70 transition-opacity shrink-0 ${isStorm ? "text-white" : "text-ink"}`}
      >
        <svg
      ...
      {/* Nav tabs - Only icons on mobile */}
      <div
        className={`flex items-center gap-0.5 border rounded-full p-[3px] ${isStorm ? "bg-white/10 border-white/20" : "bg-muted border-line"}`}
      >
        <Link
          href="/dashboard"
          className={clsx(
      ...
      <div className="flex items-center gap-3 shrink-0">
        <Link
          href="/profile"
          className="rounded-full overflow-hidden hover:ring-2 hover:ring-blush transition-all block focus:outline-none focus:ring-2 focus:ring-blush"
        >
          {user?.image ? (
            <img
              src={user.image}
              alt={user.name}
              className="w-8 h-8 rounded-full border-[1.5px] border-line object-cover"
