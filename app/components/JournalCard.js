...
export default function JournalCard({ onSaved }) {
  ...
  const addHighlight = (colorInfo) => {
    ...
    const newHighlights = highlights.filter(
      (h) => !(h.start < selectionPopup.end && h.end > selectionPopup.start),
    );
    ...
    try {
      const res = await fetch("/api/entries", {
        ...
        body: JSON.stringify({
          ...
          entryDate: (overrideDate
            ? new Date(
                overrideDate.split("-")[0],
                overrideDate.split("-")[1] - 1,
                overrideDate.split("-")[2],
              )
            : new Date()
          ).toISOString(),
        }),
        ...
        >
          ...
          {imageBase64 ? (
            <>
              ...
              <button
                className="absolute top-3 right-3 w-9 h-9 rounded-full bg-ink/70 shadow-md text-white flex items-center justify-center hover:bg-ink transition-colors z-10"
                onClick={(e) => {
                  e.stopPropagation();
                  openCropper();
                }}
              ...
              <div className="absolute bottom-0 left-0 right-0 flex items-center justify-center p-3 bg-gradient-to-t from-black/40 to-transparent">
                <button
                  className="px-4 py-2 rounded-full bg-black/60 text-white text-sm font-medium hover:bg-black transition-colors shadow-md"
                  onClick={(e) => {
                    e.stopPropagation();
                    clearImage();
                  }}
            ...
            <div className="flex flex-col items-center gap-3 text-ink-faint py-12">
              ...
              <div className="text-center">
                ...
                <p className="text-xs opacity-70 mt-0.5">
                  or click to browse · square crop
                </p>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 flex flex-col border-t md:border-t-0 md:border-l border-line">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-4 border-b border-line gap-4 sm:gap-2">
            <div className="flex items-center gap-3">
              ...
              {process.env.NEXT_PUBLIC_APP_MODE === "testing" ? (
                <div className="relative group shrink-0" title="Override Date">
                  ...
                  <div className="w-12 h-12 rounded-lg border-2 border-dashed border-blush/40 bg-blush-light flex flex-col items-center justify-center leading-none group-hover:bg-blush transition-colors">
                    <span className="text-[0.6rem] font-bold text-blush group-hover:text-white uppercase transition-colors">
                      {monthShort}
                    </span>
                    <span className="text-lg font-bold text-ink group-hover:text-white leading-none transition-colors">
                      {dayNum}
                    </span>
                  </div>
                </div>
                ...
                <div className="w-12 h-12 rounded-lg border-2 border-line bg-white flex flex-col items-center justify-center leading-none">
                  <span className="text-[0.6rem] font-bold text-blush uppercase">
                    {monthShort}
                  </span>
              ...
              <div>
                <p className="text-[0.95rem] font-bold text-ink leading-tight">
                  {weekday}
                </p>
          ...
          <div className="relative flex-1 bg-white/40 transition-colors focus-within:bg-white/60 min-h-[220px] p-8 select-none">
            ...
            >
              {(() => {
                ...
                highlights.forEach((h) => {
                  segments.push({ ...h, type: "highlight" });
                });
                ...
                segments.forEach((seg, idx) => {
                  ...
                  result.push(
                    <span key={`txt-${idx}`}>
                      {text.substring(lastIdx, seg.start)}
                    </span>,
                  );
                  ...
                  <button
                    ...
                    onClick={() => {
                      const newHighlights = highlights.filter(
                        (h) =>
                          !(
                            h.start < selectionPopup.end &&
                            h.end > selectionPopup.start
                          ),
                      );
            ...
            {text.length > 0 && (
              <div className="absolute bottom-6 right-8 pointer-events-none opacity-40">
                <span className="font-[family:inherit] italic text-xs text-ink-faint">
          ...
          <div className="flex items-center justify-between px-6 py-3.5 border-t border-line gap-2.5">
            <div className="flex-1 flex items-center gap-3">
              {error && (
                <p className="text-sm text-blush font-medium">{error}</p>
              )}
