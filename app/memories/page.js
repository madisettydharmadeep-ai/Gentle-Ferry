...
function MemoriesContent() {
  ...
  const [testModeDays, setTestModeDays] = useState(isTestingMode ? 10 : 0); // Number of days to fill with test data (0 = disabled)
  ...
  const generateTestData = useCallback(() => {
    if (!isTestingMode || testModeDays <= 0) return [];

    const daysInMonth = new Date(viewYear, viewMonth, 0).getDate();
    const maxDays = Math.min(testModeDays, daysInMonth); // Don't exceed actual days in month
    const fakeEntries = [];

    for (let day = 1; day <= maxDays; day++) {
      const hasText = Math.random() > 0.15; // 85% have text
      const numEntries = Math.random() > 0.6 ? 2 : 1; // 40% chance of 2 entries
    ...
    return fakeEntries;
  }, [viewYear, viewMonth, testModeDays, isTestingMode]);

  useEffect(() => {
    if (!loading && !user) router.push("/");
  }, [user, loading, router]);

  // Generate test data whenever month/year changes (memoized to prevent infinite loops)
  const testData = useMemo(
    () => (testModeDays > 0 ? generateTestData() : []),
    [testModeDays, generateTestData],
  );
  ...
  const fetchEntries = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/entries?userId=${user.id}&year=${viewYear}&month=${viewMonth}${searchParam}`,
      );
      const data = await res.json();
      if (data.success) realEntries = data.entries;
    } catch {}

    // Merge real entries with test data (real entries take priority)
    const currentTestData = testDataRef.current;
    if (testModeDays > 0 && currentTestData.length > 0) {
      const realEntryDates = new Set(
        realEntries.map((e) => new Date(e.entryDate).getDate()),
      );
      const filteredTestData = currentTestData.filter(
        (e) => !realEntryDates.has(new Date(e.entryDate).getDate()),
      );
      setEntries([...realEntries, ...filteredTestData]);
    } else {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.text(`${MONTHS[viewMonth - 1]}`, pageWidth / 2, 85, {
        align: "center",
      });
      ...
      pdf.text("A Collection of Memories", pageWidth / 2, 130, {
        align: "center",
      });
      ...
      const statsY = 155;
      pdf.text(
        `${entries.length} entries  ·  ${totalWords.toLocaleString()} words`,
        pageWidth / 2,
        statsY,
        { align: "center" },
      );
      ...
      if (sortedMoods.length > 0) {
        ...
        pdf.setFontSize(9);
        pdf.setTextColor(140, 130, 120);
        pdf.text("Mood Distribution", pageWidth / 2, chartY - 5, {
          align: "center",
        });
        ...
        sortedMoods.slice(0, 4).forEach(([mood, count]) => {
          ...
          pdf.text(
            `${mood} (${Math.round((count / totalMoodEntries) * 100)}%)`,
            legendX + 6,
            legendY + 3,
          );
          legendX += 35;
      ...
      pdf.setFontSize(11);
      pdf.setTextColor(120, 110, 100);
      pdf.text("Written by", pageWidth / 2, pageHeight - 35, {
        align: "center",
      });
      ...
      pdf.setFontSize(14);
      pdf.setTextColor(50, 45, 40);
      pdf.text(user?.name || "Anonymous", pageWidth / 2, pageHeight - 25, {
        align: "center",
      });
  ...
  // Fetch real entries and merge with test data
  useEffect(() => {
    if (user) fetchEntries();
  }, [fetchEntries]);

  // When testData changes (month switch), update entries with test data if no fetch needed
  useEffect(() => {
    if (testModeDays > 0 && testData.length > 0 && user) {
      fetchEntries();
    }
  }, [testData]);
  ...
  const totalWords = entries.reduce(
    (acc, e) => acc + (e.text?.split(/\s+/).filter(Boolean).length || 0),
    0,
  );
  ...
  const handleDeleteEntry = async (entryId) => {
    try {
      const res = await fetch(`/api/entries?id=${entryId}&userId=${user.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
      ...
      <main className="flex-1 max-w-[1060px] mx-auto px-2 md:px-6 py-12 md:pb-20 w-full">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 relative z-10">
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-150 z-50">
                <div className="bg-white/80 backdrop-blur-xl border border-white/40 shadow-xl rounded-lg px-4 py-3 flex items-center gap-4 whitespace-nowrap">
                  <div className="text-center">
                    ...
                    <p className="text-[0.6rem] text-ink-faint uppercase tracking-wider font-bold mt-0.5">
                      entries
                    </p>
                  </div>
                  ...
                  <div className="text-center">
                    <p className="text-base font-bold text-ink leading-none">
                      {entries.filter((e) => e.driveImageUrl).length}
                    </p>
                    <p className="text-[0.6rem] text-ink-faint uppercase tracking-wider font-bold mt-0.5">
                      photos
                    </p>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            {/* Test Mode Days input - only show in testing mode */}
            {isTestingMode && (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-16 px-2 py-1 text-sm rounded-lg bg-white/20 backdrop-blur-md border border-white/30 text-ink focus:outline-none focus:bg-white/40 focus:border-blush/50 transition-all"
                  title="Number of days to fill with test data (0 = disabled)"
                />
              </div>
            )}
        ...
        <div className="flex items-center gap-1 bg-white/20 backdrop-blur-3xl border border-white/30 p-1 rounded-xl shadow-xl w-full md:w-auto mb-6">
          <button
            ...
            className={clsx(
              "flex-1 md:flex-auto px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2",
              activeTab === "calendar"
                ? "bg-white shadow-md text-ink"
            ...
            {!isSearching && entries.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {entries.map((entry, idx) => {
                  const date = new Date(entry.entryDate);
                  const thumbUrl = entry.driveImageUrl
                    ? getImageSrc(entry)
                    : null;
                  const textPreview =
                    entry.text?.replace(/[@#]\w+/g, "").slice(0, 120) || "";
          ...
          !searchQuery.trim() && (
            <div className="flex flex-col items-center gap-2 py-16 px-6 text-center w-full mt-10 bg-white/20 backdrop-blur-3xl border border-white/30 shadow-xl rounded-lg relative z-10">
              ...
              <h3 className="text-ink text-lg font-bold">
                No entries for {MONTHS[viewMonth - 1]} {viewYear}
              </h3>
              <p className="text-ink-soft text-sm mb-3">
                A blank canvas awaits. Capture a quiet thought or a sudden
                feeling.
              </p>
              <button
                className="px-6 py-2.5 rounded-full bg-ink text-white shadow-md text-sm font-bold transition-all hover:bg-blush hover:-translate-y-0.5 active:scale-95"
        ...
        {activeTab === "people" && peopleList.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-16 px-6 text-center w-full mt-10 bg-white/20 backdrop-blur-3xl border border-white/30 shadow-xl rounded-lg relative z-10">
            ...
            <p className="text-ink-soft text-sm mb-3">
              Include friends in your journal by typing{" "}
              <strong className="text-blue-500">@name</strong> inside your
              entries.
            </p>
          </div>
        )}
        ...
        {activeTab === "places" && placesList.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-16 px-6 text-center w-full mt-10 bg-white/20 backdrop-blur-3xl border border-white/30 shadow-xl rounded-lg relative z-10">
            ...
            <p className="text-ink-soft text-sm mb-3">
              Record the places you visit by typing{" "}
              <strong className="text-emerald-600">#place</strong> inside your
              entries.
            </p>
          </div>
        )}
      ...
      {selectedEntries && (
        ...
        >
          ...
          >
            ...
            {/* ── MOVED NAVIGATION CONTROLS HERE ── */}
            {/* Vertical Up/Down Controls — only if multiple */}
            {selectedEntries.length > 1 && (
              <>
                {/* Adjusted right-2 for mobile, -right-20 for desktop */}
                ...
                {/* Adjusted left-0 for mobile, -left-20 for desktop */}
